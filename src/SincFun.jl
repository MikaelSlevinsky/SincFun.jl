...
Base.getindex{D<:Domain,S<:Number,T<:Number}(sf::sincfun{D,S},x::AbstractVector{T}) = promote_type(S,T)[sf[xk] for xk in x]
Base.getindex{D<:Domain,S<:Number,T<:Number,N}(sf::sincfun{D,S},x::AbstractArray{T,N}) = reshape(sf[vec(x)],size(x))
